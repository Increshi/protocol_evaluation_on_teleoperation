...
public class IsaacUR3Client : MonoBehaviour
{
    ...
    private TcpClient _stateTcpClient;
    ...
    void Start()
    {
        ...
        UpdateStatus("Ready (UDP cmd + TCP state)...", Color.white);
    ...
    public void Connect()
    {
        ...
        try
        {
            ...
            if (_stateTcpClient != null) _stateTcpClient.Close();
            _stateTcpClient = new TcpClient();
            _stateTcpClient.NoDelay = true;
            _stateTcpClient.ReceiveBufferSize = 8192;
            _stateTcpClient.Connect(IPAddress.Parse(ip), listenPort);
            _stateStream = _stateTcpClient.GetStream();
            ...
            Debug.Log($"✅ Connected! TCP state on {listenPort}, UDP cmd on {sendPort}");
    ...
    private void ReceiveLoop()
    {
        ...
        while (_isRunning)
        {
            try
            {
                if (_stateStream == null) break;
                int bytesRead = _stateStream.Read(buffer, 0, buffer.Length);
                ...
                while (true)
                {
                    ...
                    // T6: Unity UTC ms when a full JSON line is received
                    long t6 = DateTimeOffset.UtcNow.ToUnixTimeMilliseconds();
                    lock (_queueLock)
                    {
                        _receivedQueue.Enqueue(line);
                        _pendingT6 = t6;
                    }
                }
            }
            ...
            {
                // Stream disposed while blocking on Read; exit loop
                break;
            }
            catch (Exception ex)
            {
    ...
    public void SendDelta(int index, float amount, long t0 = 0)
    {
        if (_udpSender != null)
        {
            ...
            try { _udpSender.Send(bytes, bytes.Length, _serverEndPoint); } catch {}
        }
    }
    ...
    public void DisconnectFromServer()
    {
        _isRunning = false;
        StopContinuousDelta();
        try { if (_stateStream != null) _stateStream.Close(); } catch (Exception) { }
        try { if (_stateTcpClient != null) _stateTcpClient.Close(); } catch (Exception) { }
        try { if (_videoReceiver != null) _videoReceiver.Close(); } catch (Exception) { }
        try { if (_udpSender  != null) _udpSender.Close();  } catch (Exception) { }
        CmdToExpected.Clear();
    ...
    void OnApplicationQuit()
    {
        ...
        try
        {
            if (_stateStream != null) _stateStream.Close();
        }
        catch (Exception) { }
        ...
        try
        {
            if (_udpSender != null) _udpSender.Close();
        }
        catch (Exception) { }
